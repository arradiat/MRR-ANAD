...
```{r}
#\begin{align*}
#\ln(P_{\theta}(X,Z))=\ln(P_{\theta}(X/Z))+\ln(P_{\theta}(Z))
#&=\ln(\prod\limits_{i=1}^n \prod\limits_{k=1}^K P_{\theta}(X_i/Z_i=k)^{Z_{ik}})+\ln(\prod\limits_{i=1}^n #\prod\limits_{k=1}^K P_{\theta}(Z_i=k)^{Z_{ik}})\\
#&=\ln(\prod\limits_{i=1}^n \prod\limits_{k=1}^K P_{\theta}(X_i/Z_i=k)^{Z_{ik}})+\ln(\prod\limits_{i=1}^n \prod\limits_{k=1}^K \pi_k^{Z_{ik}})\\
#&=\sum\limits_{i=1}^n \sum\limits_{k=1}^K Z_{ik}[\ln( P_{\theta}(X_i/Z_i=k))+\ln(\pi_k)]\\
#\end{align*}
```
...
$$\mathbb{E}[\ln(P_{\theta^{q+1}}(Z|X))]=\mathbb{E}[\ln({P_{\theta^{q+1}}(X,Z)})-\ln({P_{\theta^{q+1}}(X)})]$$
$$\mathbb{E}[\ln(P_{\theta^{q+1}}(Z|X))]=\mathbb{E}[\ln({P_{\theta^{q+1}}(X,Z)})-\ln({P_{\theta^{q+1}}(X)})]$$
$$-\mathbb{E}[\ln(P_{\theta^{q+1}}(Z|X))]=-\mathbb{E}[\ln({P_{\theta^{q+1}}(X,Z)})]+\ln({P_{\theta^{q+1}}(X)})$$
$$-\mathbb{E}[\ln(P_{\theta^{q+1}}(Z|X))]=-\mathbb{Q}(\theta^{q+1}|\theta)+\ln({P_{\theta^{q+1}}(X)})$$
```{r, include=FALSE}
#\begin{align*}
#-\mathbb{E}_{/X}(ln(P(Z/X,\theta^{q+1})))&=-\mathbb{E}_{/X}(ln(P(Z,X,\theta^{q+1})))+\mathbb{E}_{/X}(ln(P(X,\theta^{q+1})))\\
#&=-\mathbb{Q}(\theta^{q+1}/\theta)+\ln(P_{\theta^{q+1}}(X))\\
#\end{align*}
```
...
## Question 9:
$$\hat k_{ICL}=\smash{\mathop{{\rm argmax}}\limits_{k}}\,(\ln(P_{\hat{\theta}}(X))-d_k\ln (n)+\mathbb{E}_{Z|X,\theta}[\ln(P_\hat \theta(Z|X) )])$$
le terme$\mathbb{E}_{Z|X,\theta}[\ln(P_\hat \theta(Z|X) )]$ représente l'entropie de la partition 
$$\hat k_{ICL}=\smash{\mathop{{\rm argmax}}\limits_{k}}\,(\mathbb{E}_{Z|X,\theta}[\ln(P_\hat \theta(X,Z) )]-\frac{d_k\ln (n)}{2})$$
```{r,include=FALSE}
#\begin{align*}
#\hat{K}_{ICL}&=\smash{\mathop{{\rm argmax}}\limits_{k}}\,(\hat{K}_{BIC}-\mathbb{E}_{/X}(ln(P(Z/X,\theta^{q+1}))))\\
#&=\smash{\mathop{{\rm argmax}}\limits_{k}}\,(\hat{K}_{BIC}-\mathbb{Q}(\theta^{q+1}/\theta)+\ln(P_{\theta^{q+1}}(X)))\\
#\end{align*}
```
